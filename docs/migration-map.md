...
| Add-in push/pull | `packages/office-core` and API artifact flow | Exact compressed capture, authenticated document binding, base-aware upload/finalize, conflict preservation, cancellation, and processing status are implemented; authorized pull/open remains later work | 13 push complete |
