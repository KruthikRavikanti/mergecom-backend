...
| Add-in push/pull | `packages/office-core` and API artifact flow | Exact compressed capture, authenticated document binding, base-aware upload/finalize, conflict preservation, exact-version selection, verified separate-file open, and authorized original download replace every reconstruction path | 14 retrieval complete |
